...
MIN_BAND_GAP = 3
PERC_MODE = "multi"
RATIO_TOTAL_CUBES_FOR_CONTRAST = 0.75 #i.e. if = 0.75; then first 75% of cubes in sorted path list used to sample percentile for contrast
VAR_STRIDE = 3 #length of each step inbetween each contrast sample per cube's band

OUTPUT_MODE = "grayscale" #"grayscale" #; anything but "RBG" is grayscale
...
def prefix_before_z_token(path: Path) -> str:
    stem = path.stem
    key_z = "expected_z_position_"
    key_row = KEY_ROW

    z_pos = stem.find(key_z)
    if z_pos == -1:
        return ""

    row_pos = stem.find(key_row, z_pos)
    if row_pos == -1:
        return ""

    # return from key_z (included) up to KEY_ROW (excluded)
    return stem[z_pos:row_pos]
...
def hyperspectral_to_rgb(cube, bands_idx, p1, p9):
    ...
    for c, b in enumerate(bands_idx):
        ...
        if TEST_MANUAL_CONTRAST:
            denom = (TEST_P9[c] - TEST_P1[c]) + 1e-8
            norm = (ch - TEST_P1[c]) / denom
        norm = clamp01(norm)
        if GAMMA != 1.0:
            norm = norm ** (1.0 / GAMMA)
        chans.append(norm)
    ...
    if OUTPUT_MODE == "grayscale":
        arr = (chans[0] + chans[1] + chans[2]) / 3.0
...
def main():
    ...
    cur_height = 0
    z_height:str = str(cur_height) if USE_Z_STACK_SHEETS else ""
    
    label_text = f"Ultra-fast mosaic {mode_tag}, {rows}x{cols}, z_height{z_height}, (mode={OUTPUT_MODE}, snake={S_PATTERN})"
    if LABEL_MODE == True:

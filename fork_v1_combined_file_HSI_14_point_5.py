#fork_v1_combined_file_HSI_14_point_5.py ; 12/03/25
...
ACCEL_TIME: float = 50 #in ms     Constants for class (global reference currently)
...
class MS2000(SerialPort):
 ...
 ##### STAGE COMMANDS USING ASI'S MS2000 STAGE COMMAND API #####
    ...
    def turn_on_light(self):
        self.change_light_power(percentage_power= 0)  # POWER IS A [0, 99] NOT 100 :^)
...
# ------------------------- #
# Cube binning helper ; unsure if need new class
# ------------------------- #
    ...
    def scan_tile(self, row, col, bin_factors, save_folder, delta_x: float,  home_points_for_naming=None, actual_super_grid_coords: tuple = ()):
        ...
        # Throttle status polls a bit (serial is slow). Tune 2..8; 4 is a safe start.
        poll_div = 2
        

        if USER_CONTINOUS_SECONDARY_CAMERA_SNAPSHOT == True:
            self._snapshot_event.set()
...
high prio/debug:
    ...
    ==> **ADD POWER SLIDER FOR BRIGHTFIELD LIGHT**
...
An instance method binds the current object in memory (self).

A class method binds the class object (cls).

A static method binds to nothing — it’s just a plain function living in the class namespace.
